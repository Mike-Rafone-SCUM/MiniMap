...
& $compiler /nologo /target:exe /platform:x64 /main:InputVoiceTests "/out:$exe" /reference:System.Drawing.dll /reference:System.Windows.Forms.dll "/resource:$root\resources\roads.bin,roads.bin" "/resource:$root\resources\water-mask.bin,water-mask.bin" @resources @sources (Join-Path $root 'tests\InputVoiceTests.cs')
