...
foreach($test in @('Test-UpdateChecker.ps1','Check-Stability.ps1')) {
...
$text=$text -replace 'AssemblyInformationalVersion\("[^"]+"\)', 'AssemblyInformationalVersion("1.4.7-responsiveness-test.2")'
...
try {
    & $compiler /nologo /optimize+ /target:winexe /platform:x64 /define:MINIMAP_TEST "/out:$exe" /reference:System.Drawing.dll /reference:System.Windows.Forms.dll "/resource:$res\map.png,map.png" "/resource:$res\zones.tsv,zones.tsv" "/resource:$res\roads.bin,roads.bin" "/resource:$res\scummap.bin,scummap.bin" "/resource:$pack\detect_zones.py,detect_zones.py" "/win32icon:$res\App-Icon.ico" @sources
...
Compress-Archive -LiteralPath $files -DestinationPath (Join-Path $output 'SCUM-MiniMap-Responsiveness-Test.zip')
