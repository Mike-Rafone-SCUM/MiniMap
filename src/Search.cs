...
namespace ScumMiniMap {
    public static class DestinationSearch {
        ...
        public static string Sector(PointF p) {
            double worldX=617718-p.X*1521618, worldY=618618-p.Y*1523618;
        ...
        public static double Metres(PointF a,PointF b) {
            double x=(a.X-b.X)*15216.18,y=(a.Y-b.Y)*15236.18; return Math.Sqrt(x*x+y*y);
        }
        public static string Distance(double metres) { return metres>=1000?(metres/1000).ToString("F1",CultureInfo.InvariantCulture)+" km":Math.Round(metres).ToString(CultureInfo.InvariantCulture)+" m"; }
        sealed class Match { internal MapZone Zone; internal int Score,Index; internal double Distance; }
        public static List<MapZone> Find(List<MapZone> places,string query,PointF? origin) {
        ...
        public static List<MapZone> FindScumMap(ScumMapStore scumMap, string query, PointF? origin, int maxResults = 30) {
            List<MapZone> result = new List<MapZone>();
            if (scumMap == null || !scumMap.MasterEnabled) return result;
            string filter = Normalize(query ?? "");
            if (filter.Length > 120) return result;
            bool nearest = (Regex.IsMatch(filter, @"\b(nearest|near|cerca|cercano|cercana)\b") || filter.Length == 0) && origin.HasValue;
            MatchCollection grids = Regex.Matches(filter, @"\b[dcbaz]\s*[0-4]\b");
            string sector = null;
            if (grids.Count == 1) {
                sector = grids[0].Value.Replace(" ", "").ToUpperInvariant();
                filter = filter.Remove(grids[0].Index, grids[0].Length).Trim();
            }
            filter = Regex.Replace(filter, @"\b(grid|sector|square|in|near|nearest|cuadricula|en|cerca|cercano|cercana)\b", "");
            filter = Regex.Replace(filter, @"\s+", " ").Trim();

            List<Match> matches = new List<Match>();
            int index = 0;
            foreach (ScumMapMarker m in scumMap.Markers) {
                if (!scumMap.IsCategoryEnabled(m.CategoryId)) continue;
                PointF pt = m.Point;
                if (sector != null && Sector(pt) != sector) continue;
                string name = scumMap.GetMarkerDisplayName(m);
                ScumMapCategory cat;
                scumMap.CategoryById.TryGetValue(m.CategoryId, out cat);
                string catName = cat != null ? (cat.Name ?? "") : "";
                string secName = cat != null ? (cat.Section ?? "") : "";

                int score = -1;
                if (filter.Length == 0) {
                    score = 0;
                } else {
                    score = Rank(Normalize(name), filter);
                    if (!string.IsNullOrEmpty(m.Title) && !string.Equals(m.Title, name, StringComparison.OrdinalIgnoreCase)) {
                        int tScore = Rank(Normalize(m.Title), filter);
                        if (tScore >= 0 && (score < 0 || tScore < score)) score = tScore;
                    }
                    if (!string.IsNullOrEmpty(catName)) {
                        int cScore = Rank(Normalize(catName), filter);
                        if (cScore >= 0 && (score < 0 || cScore < score)) score = cScore;
                        string locCat = Localization.GetCategoryName(catName);
                        if (!string.IsNullOrEmpty(locCat) && locCat != catName) {
                            int locScore = Rank(Normalize(locCat), filter);
                            if (locScore >= 0 && (score < 0 || locScore < score)) score = locScore;
                        }
                    }
                    if (!string.IsNullOrEmpty(secName)) {
                        int sScore = Rank(Normalize(secName), filter);
                        if (sScore >= 0 && (score < 0 || sScore < score)) score = sScore;
                    }
                }
                if (score >= 0) {
                    MapZone mz = new MapZone {
                        Name = name,
                        Subtitle = !string.IsNullOrEmpty(catName) ? catName : secName,
                        Points = new PointF[] { pt },
                    ...
                    matches.Add(new Match {
                        ...
                        Score = score,
                        Index = index,
                        Distance = origin.HasValue ? Metres(pt, origin.Value) : 0
                    });
                }
                index++;
            }
            ...
            matches.Sort((a, b) => {
                int c = nearest ? a.Distance.CompareTo(b.Distance) : a.Score.CompareTo(b.Score);
                if (c == 0) c = nearest ? a.Score.CompareTo(b.Score) : a.Distance.CompareTo(b.Distance);
                if (c == 0) c = string.Compare(a.Zone.Name, b.Zone.Name, StringComparison.OrdinalIgnoreCase);
                return c == 0 ? a.Index.CompareTo(b.Index) : c;
            });

            for (int i = 0; i < matches.Count && i < maxResults; i++) {
                result.Add(matches[i].Zone);
            }
            return result;
        }
